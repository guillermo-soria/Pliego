    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "#71717a", fontSize: 13, marginTop: 2 }}>{quotes.length} presupuestos guardados</p>
        </div>
      </div>
          ...
          >
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: "monospace", fontSize: 18, color: "#f97316" }}>${q.totalPrice.toFixed(2)}</span>
              </div>
          ...
          {expanded === q.id && (
            <div style={{ ...card, borderTopLeftRadius: 0, borderTopRightRadius: 0, borderTop: "none", marginTop: 0 }}>
              ...
              {q.items.map((item: QuoteItem) => (
                <div key={item.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #2e2e38", fontSize: 13 }}>
                  <span style={{ color: "#a1a1aa" }}>{item.name} × {item.qty} {item.unit}</span>
                  <span style={{ fontFamily: "monospace", color: "#f97316" }}>${item.subtotal.toFixed(2)}</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 4px", borderTop: "1px solid #2e2e38", marginTop: 6 }}>
                <span style={{ fontSize: 13, color: "#71717a" }}>Costo materiales</span>
                <span style={{ fontFamily: "monospace", fontSize: 13 }}>${q.materialCost.toFixed(2)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0 10px", borderBottom: "1px solid #2e2e38" }}>
                <span style={{ fontSize: 13, color: "#71717a" }}>Ganancia ({q.markup}%)</span>
...
const card: React.CSSProperties      = { background: "#1e1e24", border: "1px solid #2e2e38", borderRadius: 14, padding: 20 };
...
const quoteRow: React.CSSProperties  = {
  ...
  padding: "14px 16px", background: "#1e1e24",
  border: "1px solid #2e2e38", borderRadius: 10, cursor: "pointer",
